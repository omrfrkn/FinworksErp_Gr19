...
public class LoginStepDefs_ysnblgn {
    ...
    @Then("the user should see the title {string}_YB")
    public void the_user_should_see_the_title(String expectedTitle) {
        BrowserUtils.waitFor(5);
        Assert.assertEquals(expectedTitle, Driver.get().getTitle());
    }

    @Then("the user should see own username as {string}_YB")
    ...
    @Then("{string} error message should appear_YB")
    ...
    @Then("{string} warning message should appear_YB")
    public void warningMessageShouldAppear(String expectedWarningMessage) {
        if(new LoginPage_ysnblgn().email.getAttribute("value").isEmpty()){
            Assert.assertEquals(expectedWarningMessage,new LoginPage_ysnblgn().email.getAttribute("validationMessage"));
